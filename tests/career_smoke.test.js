...
emprestimoCompra.diasRestantes = 28;
emprestimoCompra.jogos = 3;
const atributoAntesEmprestimo = scope.jogadores.find((item) => item.id === emprestimoCompra.jogadorId).atributos.passe || 75;
scope.processarEmprestimosDia();
assert.ok(emprestimoCompra.evolucao >= 1, 'loan performance should generate development');
assert.ok(scope.jogadores.find((item) => item.id === emprestimoCompra.jogadorId).atributos.passe >= atributoAntesEmprestimo, 'loan development should affect player attributes');
...
preparador.salario = 50000;
assert.ok(scope.calcularFolhaSalarial() >= 50000, 'staff salary should be included in payroll');
